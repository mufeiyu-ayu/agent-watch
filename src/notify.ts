...
import type { AgentWatchConfig, NormalizedNotification } from './types.js';

export async function notify(config: AgentWatchConfig, notification: NormalizedNotification): Promise<void> {
  ...
  if (notification.summary) lines.push(notification.summary);
